...
Public Const COL_GANTT_START As String = "N"  ' ガントチャートの開始列
Public Const COL_DATA_END As String = "M"     ' 入力列の終端
...
Public Const ROW_HEADER As Long = 8           ' 曜日行（ガント）/ 項目ヘッダー行（A-M列）
...
Private Const BACKUP_SHEET_NAME As String = "WBS_Backup_Lite"
Private Const BACKUP_TEMP_SHEET_NAME As String = "_WBS_Backup_Lite_tmp"
Private Const BACKUP_OLD_SHEET_NAME As String = "_WBS_Backup_Lite_old"
...
Sub SetupInazumaGantt(Optional ByVal silentMode As Boolean = False, Optional ByVal overrideStartDate As Variant = Null)
    ...
    ' タイトル・情報エリア
    ws.Range("A" & ROW_TITLE).Value = "イナズマガントチャート Lite"
    ...
    ' ヘッダー行のスタイル（8行目、A～M列）
    With ws.Range("A" & ROW_HEADER & ":" & COL_DATA_END & ROW_HEADER)
        .Font.Bold = True
    ...
    ' フィルタ自動設定 (7行目（日付行）A-M列)
    If Not ws.AutoFilterMode Then
        ws.Range("A" & ROW_DATE_HEADER & ":" & COL_DATA_END & ROW_DATE_HEADER).AutoFilter
    End If
...
Private Sub ApplyDataValidationAndFormats(ByVal ws As Worksheet, ByVal lastRow As Long)
    ...
    ' 日付列の書式
    ws.Range(COL_START_PLAN & ROW_DATA_START & ":" & COL_END_PLAN & lastRow).NumberFormat = "yy/mm/dd"
End Sub
...
' 下: A:M + 週区切り (中太)
    ...
    Next weekCol
    ApplyBorder ws.Range(COL_DATA_END & "6"), xlEdgeRight, xlContinuous, xlThin, xlColorIndexAutomatic
    ...
    ApplyBorder ws.Range(ws.Cells(7, ws.Columns(COL_DATA_END).Column), ws.Cells(7, ganttEndCol)), xlEdgeRight, xlContinuous, xlThin, xlColorIndexAutomatic
    ...
    ApplyBorder ws.Range(ws.Cells(ROW_DATA_START, 6), ws.Cells(lastRow, ws.Columns(COL_DATA_END).Column)), xlEdgeRight, xlContinuous, xlThin, xlColorIndexAutomatic
    ApplyBorder ws.Range(ws.Cells(ROW_DATA_START, 6), ws.Cells(lastRow, ws.Columns(COL_DATA_END).Column)), xlInsideVertical, xlContinuous, xlThin, xlColorIndexAutomatic
...
Private Sub EnsureSettingsCommandButtons(ByVal wsSettings As Worksheet)
    ...
    wsSettings.Range("E4").Value = "退避・復元・差分確認・サマリ作成・今週作業確認はここから実行します。"
    ...
    AddSettingsCommandButton wsSettings, "Btn_Settings_WeeklyWork", "今週作業", "CreateWeeklyWorkSheet", _
                             leftPos + buttonWidth + 23, topPos + buttonHeight + 8, buttonWidth, buttonHeight, RGB(112, 173, 71)
...
Sub EnsureSettingsSheet()
    ...
    If Trim$(CStr(wsSettings.Range("B5").Value)) = "" Then wsSettings.Range("B5").Value = False
    wsSettings.Range("C5").Value = "← Lite版では未使用"
