...
' ============================================================================
'  InazumaGantt 統合モジュール
' ============================================================================
'  4つのモジュールを統合: InazumaGantt_v2, HierarchyColor, DataMigration, SetupWizard
' ============================================================================
...
Sub SetupHierarchyColors()
    ...
    ' LV1-4の条件付き書式
    Dim rangeLV1 As Range, rangeLV2 As Range, rangeLV3 As Range, rangeLV4 As Range
    Dim cf1 As FormatCondition, cf2 As FormatCondition, cf3 As FormatCondition, cf4 As FormatCondition
    
    Set rangeLV1 = ws.Range("C" & ROW_DATA_START & ":" & COL_COLOR_END & lastRow)
    Set cf1 = rangeLV1.FormatConditions.Add(Type:=xlExpression, Formula1:="$A" & ROW_DATA_START & "=1")
    cf1.Interior.Color = COLOR_LV1
    cf1.StopIfTrue = True
    
    Set rangeLV2 = ws.Range("D" & ROW_DATA_START & ":" & COL_COLOR_END & lastRow)
    Set cf2 = rangeLV2.FormatConditions.Add(Type:=xlExpression, Formula1:="$A" & ROW_DATA_START & "=2")
    cf2.Interior.Color = COLOR_LV2
    cf2.StopIfTrue = True
    
    Set rangeLV3 = ws.Range("E" & ROW_DATA_START & ":" & COL_COLOR_END & lastRow)
    Set cf3 = rangeLV3.FormatConditions.Add(Type:=xlExpression, Formula1:="$A" & ROW_DATA_START & "=3")
    cf3.Interior.Color = COLOR_LV3
    cf3.StopIfTrue = True
    
    Set rangeLV4 = ws.Range("F" & ROW_DATA_START & ":" & COL_COLOR_END & lastRow)
    Set cf4 = rangeLV4.FormatConditions.Add(Type:=xlExpression, Formula1:="$A" & ROW_DATA_START & "=4")
    ...
    MsgBox "階層色分けの条件付き書式を設定しました！", vbInformation, "階層色分け"
    Exit Sub
...
Sub ClearHierarchyColors()
    ...
    lastRow = ROW_DATA_START + DATA_ROWS_DEFAULT - 1
    ws.Range("B" & ROW_DATA_START & ":" & COL_COLOR_END & lastRow).FormatConditions.Delete
    MsgBox "階層色分けをクリアしました！", vbInformation, "階層色分け"
    Exit Sub
ErrorHandler:
...
Sub MigrateToV2Format()
    ...
    For oldRow = 2 To lastOldRow
        If Trim$(CStr(oldSheet.Cells(oldRow, "C").Value)) <> "" Then
            ...
            MapDateColumnsForMigrate oldSheet, newSheet, oldRow, newRow
            MapProgressColumnForMigrate oldSheet, newSheet, oldRow, newRow
            MapAssigneeColumnForMigrate oldSheet, newSheet, oldRow, newRow
    ...
    MsgBox "移管完了！ 移管行数: " & (newRow - ROW_DATA_START), vbInformation, "データ移管"
    Exit Sub
...
Private Sub MapDateColumnsForMigrate(ByVal oldSheet As Worksheet, ByVal newSheet As Worksheet, ByVal oldRow As Long, ByVal newRow As Long)
    Dim col As Long
...
Private Sub MapProgressColumnForMigrate(ByVal oldSheet As Worksheet, ByVal newSheet As Worksheet, ByVal oldRow As Long, ByVal newRow As Long)
    Dim col As Long
    Dim header As String
...
Private Sub MapAssigneeColumnForMigrate(ByVal oldSheet As Worksheet, ByVal newSheet As Worksheet, ByVal oldRow As Long, ByVal newRow As Long)
    Dim col As Long
    Dim header As String
...
Sub RunSetupWizard()
    ...
    result = MsgBox("InazumaGantt セットアップウィザードへようこそ！" & vbCrLf & vbCrLf & _
                   ...
                   "続行しますか？", vbQuestion + vbYesNo, "セットアップウィザード")
    ...
    result = MsgBox("新しいシート「InazumaGantt_v2」を作成しますか？", _
                   vbQuestion + vbYesNo, "ステップ 1/3: シート作成")
    
    If result = vbYes Then
        CreateMainSheetForWizard
    End If
    
    result = MsgBox("サンプルデータを追加しますか？", vbQuestion + vbYesNo, "ステップ 2/3: サンプルデータ")
    
    If result = vbYes Then
        AddSampleDataForWizard
    End If
    ...
    MsgBox "セットアップウィザードが完了しました！", vbInformation, "セットアップ完了"
    Exit Sub
...
Private Sub CreateMainSheetForWizard()
    Dim ws As Worksheet
    On Error Resume Next
...
Private Sub AddSampleDataForWizard()
    On Error GoTo ErrorHandler
    ...
    ' フェーズ2: 開発フェーズ
    ws.Cells(startRow + 2, "C").Value = "開発フェーズ"
    ws.Cells(startRow + 2, "H").Value = "進行中"
    ws.Cells(startRow + 2, "I").Value = 0.6
    ws.Cells(startRow + 2, "K").Value = baseDate - 7
    ws.Cells(startRow + 2, "L").Value = baseDate + 14
    ws.Cells(startRow + 2, "M").Value = baseDate - 7
    
    ws.Cells(startRow + 3, "D").Value = "機能開発"
    ws.Cells(startRow + 3, "H").Value = "進行中"
    ws.Cells(startRow + 3, "I").Value = 0.5
    ws.Cells(startRow + 3, "K").Value = baseDate - 5
    ws.Cells(startRow + 3, "L").Value = baseDate + 7
    ws.Cells(startRow + 3, "M").Value = baseDate - 5
    
    ' フェーズ3: リリース
    ws.Cells(startRow + 4, "C").Value = "リリースフェーズ"
    ws.Cells(startRow + 4, "H").Value = "未着手"
    ws.Cells(startRow + 4, "I").Value = 0
    ws.Cells(startRow + 4, "K").Value = baseDate + 14
    ws.Cells(startRow + 4, "L").Value = baseDate + 21
...
Sub ShowSheetModuleInstructions()
    MsgBox "【シートモジュールの設定手順】" & vbCrLf & vbCrLf & _
          "1. Alt + F11 でVBAエディタを開く" & vbCrLf & _
          "2. プロジェクトエクスプローラーで「InazumaGantt_v2」シートをダブルクリック" & vbCrLf & _
          "3. vba/SheetModule_SJIS.bas の内容をコピー＆貼り付け" & vbCrLf & _
          "4. 保存して閉じる", vbInformation, "シートモジュール設定"
End Sub

Sub CheckInstallation()
    MsgBox "【モジュールインストール状態】" & vbCrLf & vbCrLf & _
          "統合モジュール: OK" & vbCrLf & vbCrLf & _
          "含まれる機能:" & vbCrLf & _
          "  - InazumaGantt_v2 (メイン)" & vbCrLf & _
          "  - HierarchyColor (階層色分け)" & vbCrLf & _
          "  - DataMigration (データ移管)" & vbCrLf & _
          "  - SetupWizard (ウィザード)", vbInformation, "インストール状態"
End Sub
