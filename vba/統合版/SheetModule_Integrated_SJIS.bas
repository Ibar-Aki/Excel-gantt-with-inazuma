' ==========================================
'  InazumaGantt_Integrated シートモジュール用コード
' ==========================================
' このコードは「InazumaGantt_v2」シートのシートモジュールに貼り付けてください
' ※ 統合版（InazumaGantt_Integrated）を使用する場合はこちらを使用
'
' 【設定方法】
...
' データ開始行（InazumaGantt_Integratedモジュールと同期）
...
Private Sub Worksheet_BeforeDoubleClick(ByVal Target As Range, Cancel As Boolean)
    ...
    ' 設定マスタから機能有効を確認
    If Not InazumaGantt_Integrated.GetSettingValue(3) Then Exit Sub
    ...
    ' 設定：完了日自動入力
    If InazumaGantt_Integrated.GetSettingValue(4) Then
    ...
    ' 設定：取り消し線
    If InazumaGantt_Integrated.GetSettingValue(5) Then
        Me.Range("C" & Target.Row & ":F" & Target.Row).Font.Strikethrough = True
    End If
    
    ' 設定：濃い灰色に変更
    If InazumaGantt_Integrated.GetSettingValue(6) Then
...
Private Sub Worksheet_BeforeRightClick(ByVal Target As Range, Cancel As Boolean)
    ...
    InazumaGantt_Integrated.ToggleTaskCollapse Target.Row
    Cancel = True
    Exit Sub
        ...
        For Each cell In Intersect(Target, Me.Range("C:F"))
            If cell.Row >= ROW_DATA_START Then
                ...
                If Trim$(CStr(cell.Value)) <> "" Then
                    ' 階層を自動判定
                    InazumaGantt_Integrated.AutoDetectTaskLevel cell.Row
