...
- Excel のステータスバーはアプリ全体で共有されるため、セル選択時や入力時の左下メッセージは表示しません。`ガント更新` 中だけ進行状況を一時表示し、完了時に消去します。
...
- Lite 版は `WBS差分確認` で `WBS_Backup_Lite` と現状の値差分を `WBS差分確認_Lite` に出力できます。
...
- 下位タスクに `!!` または `!` が付いた場合は、LV2-LV4 の上位タスクへ同じ注意マークを伝播します。LV1 には注意マークを表示しません。
- 親タスクの `遅延` 判定は、その親行に表示されている完了予定日を基準にします。親行のガント予定線が今日以降なら、配下に過去予定があっても親行自体は遅延にしません。
- `ガント更新` は二重実行を抑止し、更新中はステータスバーへ進行状況を一時表示して Excel の応答を維持します。
- 通常入力時の親集計と警告更新は、変更行と祖先チェーンだけを再計算して全体走査を避けます。C-F の単一セル文言編集では、タスク有無が変わらない限り全体再採番も抑制します。
- 予定日列は通常の日付だけでなく Excel シリアル値として入った日付も警告判定に利用します。
- 親タスクの予定日は、空欄なら `ガント更新` で配下タスクから自動補完し、手入力した親予定日は保持します。空欄へ戻すと自動計算へ復帰します。
- イナズマ線は接続点をガント領域内へクランプし、線幅ぶんの図形はみ出しを抑制します。
...
| Lite版 WBS差分確認 | `WBS_Backup_Lite` と現状の差分をタスクパス単位で一覧化 |
...
| `CreateWbsBackupDiffSheet` | Lite 版で `WBS_Backup_Lite` と現状の差分を `WBS差分確認_Lite` に出力 |
