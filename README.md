...
- Excel のステータスバーはアプリ全体で共有されるため、ガント由来の左下メッセージは表示しない仕様にしました。進行状態はシート上の表示、ボタン、完了ダイアログで確認します。
...
- 下位タスクに `!!` または `!` が付いた場合は、上位タスクにも同じ注意マークを伝播します。複数の注意が混在する場合は `!!` を優先します。
- `今週作業` ボタンを追加しました。進行中、未着手かつ遅延、今週するはずの未着手をカテゴリ順に抽出した `今週作業` シートを作成します。
- 通常入力時の親集計と警告更新は、変更行と祖先チェーンだけを再計算して全体走査を避けます。C-F の単一セル文言編集では、タスク有無が変わらない限り全体再採番も抑制します。
- 予定日列は通常の日付だけでなく Excel シリアル値として入った日付も警告・今週作業判定に利用します。
...
| 今週作業 | 進行中、未着手かつ遅延、今週するはずの未着手をカテゴリ順に抽出 |
...
| `CreateWeeklyWorkSheet` | 今週作業シートを生成 |
