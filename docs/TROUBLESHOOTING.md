...
標準モジュールは `vba/` フォルダ内の **`_SJIS.bas`** ファイルをインポートしてください。

| ファイル | 用途 |
|----------|------|
| `_SJIS.bas` | ✅ Excelにインポート用 |
| `_UTF8.bas` | ✅ 編集・シートモジュールの手動貼り付け用 |

> `SheetModule*.bas` を手動で貼り付ける場合は `_UTF8.bas` を使ってください。
...
3. `vba/SheetModule_UTF8.bas` の内容を貼り付け
4. 保存して閉じる
...
2. `vba/SheetModule_UTF8.bas` の最新版を使用

---

## タスク名を消したら周辺情報まで消えてしまう

### 症状

`C:F` のタスク名を消したときに、開始予定、開発LT、状況、進捗率などまで消えてしまう

### 解決方法

最新版では、`C:F` が空になっても `G:O` 側の補助情報が残っていれば、その行は有効行として保持されます。

- `C` 列には `（補助情報のみ）` が表示されます
- `LV` は見た目上は空欄ですが、親集計と `WBSサマリ` のために直前の階層を内部保持します
- `No.` は補助情報が残っている行なら維持されます
- 親集計・進捗率・開発LT・日付範囲・WBSサマリもこの行を含めて再計算します
- 完全に空行になったときだけ派生表示がクリアされます

---

## 高速入力 ON 中に見た目がすぐ更新されない

### 症状

「高速入力 ON」の間、親集計やガントの見た目が即時にそろわない

### 原因

高速入力モードでは、LV と No. は即時更新しつつ、親集計・色分け・ガントの反映は保留する

### 解決方法

1. すぐに全体反映したい場合は「高速入力 OFF」に戻す
2. 手動で確定したい場合は `RefreshInazumaGantt` を実行する
