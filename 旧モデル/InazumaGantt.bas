Attribute VB_Name = "InazumaGantt"
Option Explicit
...
' ==========================================
' ユーザー指定のレイアウト:
' A: No. | B: タスクの階層 | C: タスク | D: タスクの詳細 | E: タスクの状況
' F: 進捗率 | G: 担当 | H: 開始予定 | I: 完了予定 | J: 開始実績 | K: 完了実績
' L以降: ガントチャート領域 (日付)

Public Const COL_NO As String = "A"
Public Const COL_HIERARCHY As String = "B"
Public Const COL_TASK As String = "C"
Public Const COL_TASK_DETAIL As String = "D"
Public Const COL_STATUS As String = "E"
Public Const COL_PROGRESS As String = "F"
Public Const COL_ASSIGNEE As String = "G"
Public Const COL_START_PLAN As String = "H"
Public Const COL_END_PLAN As String = "I"
Public Const COL_START_ACTUAL As String = "J"
Public Const COL_END_ACTUAL As String = "K"

Public Const COL_GANTT_START As String = "L"  ' ガントチャートの開始列
...
Public Const MAIN_SHEET_NAME As String = "InazumaGantt"
Public Const GUIDE_LEGEND_START_CELL As String = "E1"
Public Const CELL_PROJECT_START As String = "G3"
Public Const CELL_DISPLAY_WEEK As String = "G4"
Public Const CELL_TODAY As String = "I3"
...
Sub SetupInazumaGantt()
    ...
    ws.Range("F3").Value = "プロジェクトの開始:"
    ws.Range("F4").Value = "週表示:"
    ws.Range("H3").Value = "今日:"
    
    ' ヘッダー設定 (ROW_HEADER_LABELに統一)
    ws.Range(COL_NO & ROW_HEADER_LABEL).Value = "No."
    ws.Range(COL_HIERARCHY & ROW_HEADER_LABEL).Value = "階層"
    ws.Range(COL_TASK & ROW_HEADER_LABEL).Value = "タスク"
    ...
    ' ヘッダー行のスタイル
    With ws.Range("A" & ROW_HEADER_LABEL & ":K" & ROW_HEADER_LABEL)
        .Font.Bold = True
    ...
    ReDim dateValues(1 To 1, 1 To GANTT_DAYS)
    
    Dim i As Long
    For i = 1 To GANTT_DAYS
        dateValues(1, i) = ganttStartDate + i - 1
    Next i
    
    ws.Range(ws.Cells(ROW_HEADER, ganttStartCol), ws.Cells(ROW_HEADER, ganttStartCol + GANTT_DAYS - 1)).Value = dateValues
    
    ' 日付セルの書式 (一括適用)
    With ws.Range(ws.Cells(ROW_HEADER, ganttStartCol), ws.Cells(ROW_HEADER, ganttStartCol + GANTT_DAYS - 1))
        .NumberFormat = "d"
        .Font.Size = 8
        .ColumnWidth = 3
    ...
    For i = 1 To GANTT_DAYS
        ...
        ' 曜日表示（ROW_HEADER_LABEL の L 以降）
        ws.Cells(ROW_HEADER_LABEL, colIndex).Value = Format$(currentDate, "aaa")
        ws.Cells(ROW_HEADER_LABEL, colIndex).Font.Size = 8
        ws.Cells(ROW_HEADER_LABEL, colIndex).HorizontalAlignment = xlCenter
