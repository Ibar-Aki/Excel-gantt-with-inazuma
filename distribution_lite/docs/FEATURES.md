...
| C列アラート | 期限超過は `!!`、今週予定は `!` を表示。下位タスクの注意は上位タスクへ伝播し、混在時は `!!` を優先 |
...
| 今週作業 | `今週作業` ボタンで、進行中、未着手かつ遅延、今週するはずの未着手をカテゴリ順に抽出 |
...
| 今週作業 | `CreateWeeklyWorkSheet` | メイン WBS から今週確認すべき作業を抽出。カテゴリは `進行中`、`未着手かつ遅延`、`今週するはずの未着手` の順 |
...
| 俯瞰 | `CreateWeeklyWorkSheet` | 今週作業シート作成 |
...
- `CreateWeeklyWorkSheet` は既存の `今週作業` シートを作り直します。通常操作では完了通知を出し、自動テストモードでは無人実行を止めないため通知を抑制します。
- 予定日判定は通常の日付入力に加え、Excel シリアル値として入った日付も扱います。貼り付けや COM 実行で数値として入った場合も `!!` / `!` と今週作業抽出の対象になります。
