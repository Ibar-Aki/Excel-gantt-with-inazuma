...
try {
    ...
    Invoke-CoreSmokeMacros $excel $wb "InazumaGantt_Lite"
    Remove-WorksheetIfExists $excel $wb "WBS_Backup_Lite" "InazumaGantt_Lite"
    ...
    if ($deleteSheets.Count -gt 0) {
        $wb.Worksheets.Item("InazumaGantt_Lite").Activate() | Out-Null
